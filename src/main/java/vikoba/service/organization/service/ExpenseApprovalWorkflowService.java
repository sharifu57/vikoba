...
public class ExpenseApprovalWorkflowService {
    public static final String ACTION = "EXPENSE_APPROVAL";
    ...
    public List<ShareApprovalStepConfig> get(Long groupId) {
        ...
        if (nodes.isEmpty()) return List.of(new ShareApprovalStepConfig(GroupRole.ACCOUNTANT, "Accountant review"));
    ...
    public void configure(VikobaGroup group, List<ShareApprovalStepConfig> steps) {
        if (steps == null || steps.isEmpty() || steps.size() > 10)
            throw new IllegalArgumentException("Choose between 1 and 10 expense approval steps");
        var roles = new HashSet<GroupRole>();
        for (var step : steps) {
            if (step == null || step.role() == null || step.role() == GroupRole.MEMBER || !roles.add(step.role()))
                throw new IllegalArgumentException("Use a different reviewer role for each expense approval step");
        }
        ...
        for (int i = 0; i < steps.size(); i++) {
            ...
            node.setRequiredPermission("EXPENSE_APPROVE");
